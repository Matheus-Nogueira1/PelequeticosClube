# Cronograma 26/08/2026
## Introdução:
Estamos chegando nos momentos finais do prazo, onde devemos apresentar algo relevante sobre o projeto, seguindo abaixo o que devemos finalizar durante esse 1 mês.

## O que deverá ser feito durante esse 1 mês?
- **Logo "PELECO PRODUCTIONS":** 🟢 *NÍVEL DE PRIORIDADE*
 
Preparar o design da logo do projeto

Inseri-la como a primeira tela do jogo
#
- **A "Velha do Poço":** 🔴 *NÍVEL DE PRIORIDADE*

Preparar as animações, onde ela empurra o personagem ao poço

Programar a ação de empurrar dela

Alterar sprites, ao qual o player escolher
#
- **As Cutscenes:** 🔴 *NÍVEL DE PRIORIDADE*

Criar Cutscene do player caindo no poço

Criar Cutscene do player lutando
#
- **Mapa Bastilha:** 🔴 *NÍVEL DE PRIORIDADE*

Criação do design do mapa

Colisão dos objetos

Criação de interação com o cenário
#
- **NPC's:** 🔴 *NÍVEL DE PRIORIDADE*
  
Interação com o "Dummy" 

Interação com a "FireKeeper"
#
- **Inimigos:** 🟡 *NÍVEL DE PRIORIDADE*
  
Criar o primeiro inimigo teste "Dummy"
# 
- **Tela combate:** 🟡 *NÍVEL DE PRIORIDADE*
  
Terminar design

Linkar personagens
#
- **Menu** 🟢 *NÍVEL DE PRIORIDADE*
  
Programar Tela de menu

Design Tela Menu

Tela Opções

Tela Créditos

Botão Sair
#
## Observações:
🔴 PRIORIDADE **ALTA**
- Super necessário para o jogo ser terminado

🟡 PRIORIDADE **MÉDIA**
- Precisa ser terminado, mas não tem tanto foco no momento

🟢 PRIORIDADE **BAIXA**
- Pode ser deixado por ultimo
